...
(function($, d, w) {
    ...
    var isMac = w.navigator.platform == 'MacIntel',
        ...
        bubble = {
            ...
            updatePos: function(editor, elem) {
                // ugly hack by catness: move the bubble relative to the modal where it's used (dayNotesContent)
                var posModal = $("#dayNotesContent").position();
                //console.log("posModal = " + JSON.stringify(posModal));

                var sel = w.getSelection(),
                    range = sel.getRangeAt(0),
                    boundary = range.getBoundingClientRect();
                var left = posModal.left;
                var top = posModal.top;
                var  bubbleWidth = elem.width(),
                    ...
                    //};
                     var pos = {
                        x: (left + boundary.width / 2) - (bubbleWidth / 2),
                        y: top - bubbleHeight - 8
                    };
            ...
            checkForFormatting: function(currentNode, formats) {
                var validFormats = ['b', 'i', 'u', 'ol', 'ul', 'li', 'a']; // h1 h2 removed
        ...
        actions = {
            ...
            setPlaceholder: function(e) {
                if (/^\s*$/.test($(this).text())) {
                    $(this).empty();
                    /* catness : want to keep the area empty
        ...
        rawEvents = {
            keydown: function(e) {
                var elem = this;
                /* catness : commented out, it shows bubble upon pressing "a" key, why????
        ...
        events = {
            commands: {
                ...
                /* catness: it doesn't work (places all the text in h1 and it's impossible to remove)
                h1: function(e) {
                    e.preventDefault();
                ...
                h2: function(e) {
                    ...
                    if ($(window.getSelection().anchorNode.parentNode).is('h2')) {
                        d.execCommand('formatBlock', false, '<p>');s
                    } else {
    ...
    $.fn.notebook.defaults = {
        ...
        mode: 'multiline',
        modifiers: ['bold', 'italic', 'underline', 'ol', 'ul', 'anchor']  // h1 h2
    };
